...
- (void)drawRect:(CGRect)rect {
    ...
    if (![self hasTitle]) // Frames for when AlertView doesn't contain a title
        alertViewFrame = CGRectMake(self.frame.size.width/2 - ((result.width - defaultSpacing)/2),
                                    self.frame.size.height/2 - ((alertViewFrame.size.height - 180)/2),
                                    result.width - defaultSpacing,
    ...
    if (alertTextFields.count > 0)
        ...
        alertViewFrame = CGRectMake(self.frame.size.width/2 - ((result.width - defaultSpacing)/2),
                                    self.frame.size.height/2 - ((alertViewFrame.size.height - 50 + 100)/2),
                                    result.width - defaultSpacing,
    ...
    NSInteger descriptionLevel = 40.0f;
    ...
    if (![self hasTitle]) {
        ...
        descriptionLabel = [[UITextView alloc] initWithFrame:CGRectMake(15.0f,descriptionLevel + (alertViewWithVector * 20),
                                                                     alertViewFrame.size.width - 30.0f,
                                                                     60.0f)];
    }
    else
    {
        descriptionLabel = [[UITextView alloc] initWithFrame:CGRectMake(20.0f,descriptionLevel + (alertViewWithVector * 20),
                                                                     alertViewFrame.size.width - 40.0f,
                                                                     60.0f)];
    ...
    UITapGestureRecognizer *userTap=[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(userNameTap:)];
    userTap.numberOfTapsRequired=1;
    [descriptionLabel setUserInteractionEnabled:YES];
    [descriptionLabel addGestureRecognizer:userTap];
    }
    else{
        descriptionLabel.text = self.subTitle;
        
    }
    descriptionLabel.textAlignment = _messageAlignment;
...
//    descriptionLabel.lineBreakMode = NSLineBreakByTruncatingTail;
    ...
    if (_numberOfButtons == 0) { // View only contains DONE/DISMISS Button
        ...
        if (_colorScheme == nil) {
            doneButton.backgroundColor = kMarunColor;
            if (_detachButtons)
                doneButton.backgroundColor = kMarunColor;
            if (_darkTheme)
        ...
        if (_detachButtons)
            otherButton.backgroundColor = kMarunColor;
        if (_darkTheme)
        ...
        if (_detachButtons) {
            otherButton.frame = CGRectMake(8,
...
-(void)userNameTap:(UITapGestureRecognizer*)sender
{
    NSString* pressedWord = [self getPressedWordWithRecognizer:sender];
    
    NSString *strToCheck = @"request a new code";
    NSString *strToCheckVCode = @"click here";
    
    if ([pressedWord isEqualToString:@"here"])
    {
        NSLog(@"here touch");
        
        id<FCAlertViewDelegate> strongDelegate = self.delegate;
        
        if ([strongDelegate respondsToSelector:@selector(SubtitleTapAction:)]) {
            [strongDelegate SubtitleTapAction:self];
        }
    }
    else if ([strToCheckVCode containsString:pressedWord])
    {
        NSLog(@"click here touch");
    ...
    else if ([strToCheck containsString:pressedWord])
    {
        NSLog(@"request a new code touch");
...
- (void) showAlertInView:(UIViewController *)view withTitle:(NSString *)title withSubtitle:(NSString *)subTitle withCustomImage:(UIImage *)image withDoneButtonTitle:(NSString *)done andButtons:(NSArray *)buttons {
    ...
    if (_blurBackground && NSClassFromString(@"UIVisualEffectView") != nil) {
        ...
        backgroundVisualEffectView.alpha = 0.3;
